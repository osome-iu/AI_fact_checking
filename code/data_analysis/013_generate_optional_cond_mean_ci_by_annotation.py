...
- Generate the weighted group means and 95% confidence interval for the optional
    condition for both groups (belief vs. sharing) BY ANNOTATION. Also return
    the by-participant data used to generate these results. This is the same as
    script 006 but split by each annotation.
...
Outputs:
- optional_results_mean_ci_by_annotation.csv : group means and bootstrap
    confidence intervals
- optional_results_by_participant_by_annotation.csv : by-participant data used
...
if __name__ == "__main__":
    ...
    opt_ver_is_true = option_df[option_df["ano_true_false_unsure"] == "true"].copy()
    opt_ver_is_false = option_df[option_df["ano_true_false_unsure"] == "false"].copy()
    opt_ver_is_unsure = option_df[option_df["ano_true_false_unsure"] == "unsure"].copy()

    frame_list = [
        ("true", opt_ver_is_true),
        ("false", opt_ver_is_false),
        ("unsure", opt_ver_is_unsure),
    ]
    final_dfs = []
    by_condition_all_list = []
    for veracity, data in frame_list:

        # Count the number of times each participant opt'd into each condition
        # by group and headline veracity.
        by_condition_counts = (
            data.groupby(["Group", "ResponseId", "veracity"])["option_cond"]
            .value_counts()
            .to_frame("count")
            .reset_index()
            .melt(
        ...
        for participant in by_condition_counts.ResponseId.unique():
            one_p_df = by_condition_counts[
                by_condition_counts.ResponseId == participant
            ]
        ...
        # We need the below checks because there are no false headlines judged as true
        yes_col_exists = "num_yes_True" in by_condition_all.columns
        no_col_exists = "num_yes_False" in by_condition_all.columns
        by_condition_all["prop_yes_True"] = (
            by_condition_all["num_yes_True"].div(by_condition_all["num_True"])
            if yes_col_exists
            else 0
        )
        by_condition_all["prop_yes_False"] = (
            by_condition_all["num_yes_False"].div(by_condition_all["num_False"])
            if no_col_exists
            else 0
        )
        ...
        #  ...}
        tot_part_by_group = (
            data.groupby(["Group", "option_cond", "veracity"])["ResponseId"]
            .nunique()
            .to_dict()
        )
        ...
        for group_optcond, temp_data in grouped_data:
            ...
            for bool_obj in [True, False]:
                ...
                # We need this check here because there are no False headlines rated
                # as True and the weights function is provided with no weights and throws an error
                weight_sum_is_zero = True if sum(weights) == 0 else False
                if not weight_sum_is_zero:
        ...
        by_condition_all["ano_true_false_unsure"] = veracity
        by_condition_all_list.append(by_condition_all)
        check_rows(by_condition_all)

        group_mean_ci_df = pd.DataFrame.from_records(mean_value_records)

        group_mean_ci_df["ano_true_false_unsure"] = veracity

        final_dfs.append(group_mean_ci_df)

    # Concatenate the dataframes
    final_by_condition_all = pd.concat(by_condition_all_list).reset_index(drop=True)
    final_group_mean_ci = pd.concat(final_dfs).reset_index(drop=True)

    # Save output
    by_part_fname = os.path.join(
        OUTPUT_DIR, "optional_results_by_participant_by_annotation.csv"
    )
    mean_results_fname = os.path.join(
        OUTPUT_DIR, "optional_results_mean_ci_by_annotation.csv"
    )
    final_by_condition_all.to_csv(by_part_fname, index=False)
    final_group_mean_ci.to_csv(mean_results_fname, index=False)
