---
title: "2024_07_25_interactions_atai"
date: "2024_07_25"
output: 
  html_document:
    toc: true
...
# Create version WITHOUT human FC participants
belief_data_no_hfc <- belief_data[belief_data$Condition != "Human-FC",]
share_data_no_hfc <- share_data[share_data$Condition != "Human-FC",]
...
## Note: 
- The post hoc estimates are printed before conducting pairwise comparison
- Clustered standard errors were included for pairwise comparison. 
...
- [Interactions]
	- [Attitude towards AI (ATAI)]
		- [Set equations (ATAI)]
		- [Main effects w. ATAI: Belief]
		- [Main effects w. ATAI: Share]
		- [Accounting for AI judgement w. ATAI: Belief]
		- [Accounting for AI judgement w. ATAI: Share]
		- [Optional w. ATAI: Belief]
		- [Optional w. ATAI: Share]
...
optional_atai_formula = exp_response ~ option_cond*cGPT_accuracy_recode*AI_att_mean + age + edu_recoded
```

### Main effects w. ATAI: Belief
```{r me_atai_int_belief}
# We exclude the human fact checking condition here because there is no AI aspect of that experiment
mb_me_atai <- lm(me_w_atai_formula, data = belief_data_no_hfc)

print_summary(mb_me_atai)

clustered_se <- vcovCL(mb_me_atai, cluster = belief_data_no_hfc[, c("ResponseId", "qualtrics_question_num")])
round(coeftest(mb_me_atai, vcov = clustered_se),3)
```
...
```{r me_atai_int_share}
# We exclude the human fact checking condition here because there is no AI aspect of that experiment
ms_me_atai <- lm(me_w_atai_formula, data = share_data_no_hfc)

print_summary(ms_me_atai)

clustered_se <- vcovCL(ms_me_atai, cluster = share_data_no_hfc[, c("ResponseId", "qualtrics_question_num")])
round(coeftest(ms_me_atai, vcov = clustered_se),3)
```
...
mb_ai_atai <- lm(five_way_atai_formula, data = belief_data_no_opt_or_hfc)

print_summary(mb_ai_atai)

clustered_se <- vcovCL(mb_ai_atai, cluster = belief_data_no_opt_or_hfc[, c("ResponseId", "qualtrics_question_num")])
round(coeftest(mb_ai_atai, vcov = clustered_se),3)
```
...
plot_model(mb_ai_atai, type  = "pred", terms =  c( "AI_att_mean", "Condition",  "cGPT_accuracy_recode"), title = "Moderating effects of ATAI") + ylab("Believed") + theme_minimal()
```
...
ms_ai_atai <- lm(five_way_atai_formula, data = share_data_no_opt_or_hfc)

print_summary(ms_ai_atai)

clustered_se <- vcovCL(ms_ai_atai, cluster = share_data_no_opt_or_hfc[, c("ResponseId", "qualtrics_question_num")])
round(coeftest(ms_ai_atai, vcov = clustered_se),3)
```
...
plot_model(ms_ai_atai, type  = "pred", terms =  c( "AI_att_mean", "Condition",  "cGPT_accuracy_recode"), title = "Moderating effects of ATAI") + ylab("Believed") + theme_minimal()
```
...
round(coeftest(mb_opt_atai, vcov = clustered_se),3)
```
...
# Generate the estimated marginal means
test <- emtrends(mb_opt_atai, ~ option_cond*cGPT_accuracy_recode, var="AI_att_mean", infer = TRUE)
test
...
plot_model(mb_opt_atai, type  = "pred", terms =  c( "AI_att_mean", "option_cond",  "cGPT_accuracy_recode"), title = "Moderating effects of ATAI") + ylab("Believed") + theme_minimal()
```
...
# Generate the estimated marginal means
test <- emtrends(ms_ai_atai, ~ option_cond*cGPT_accuracy_recode, var="AI_att_mean", infer = TRUE)
test
...
plot_model(ms_ai_atai, type  = "pred", terms =  c( "AI_att_mean", "option_cond",  "cGPT_accuracy_recode"), title = "Moderating effects of ATAI") + ylab("Intention to share") + theme_minimal()
```
